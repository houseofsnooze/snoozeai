...
export default function Message({ name, message, fromUser }: MessageProps) {
    return (
        <button
            className={fromUser ? wrapperClasses + " ml-auto bg-blue-800/50 shadow-md shadow-yellow-600/50" : wrapperClasses + " border"}
        >
            ...
            <div className="text-xs text-muted-foreground">
                    <div className="text-xs text-muted-foreground">
                        <MarkdownPreview source={message} style={{ paddingBottom: 10, paddingTop: 0, paddingLeft: 8, paddingRight: 8, maxWidth: '80vw', borderRadius: "6px" }} />
                    </div>
            </div>
                )}
            {/* <div className="flex items-center gap-2">
